...
// Helper function to transform API menu data
const transformMenuData = (apiMenus, languageId = 1) => {
  const getTitle = (translations) => {
    const translation = translations?.find((tr) => tr.language_id === languageId) || translations?.[0];
  ...
  const transformChildren = (children) => {
    ...
    return children
      ...
      .map((child) => {
        let link = child.target || "#";
        // Convert hash-only links to home page with hash
  ...
  return apiMenus
    .filter((menu) => menu.active && menu.parent_id === null)
    .map((menu) => {
      let link = menu.target || "#";
      // Convert hash-only links to home page with hash
      if (link.startsWith("#") && link !== "#") {
        link = "/" + link;
      }
...
const MenuItemRenderer = ({ item, level = 0 }) => {
        ...
        <div style={{ breakInside: 'avoid', marginBottom: '20px' }}>
          ...
          <ul className="ps-0 mb-0">
            {item.items.map((subItem, index) => (
              <MenuItemRenderer key={`${subItem.title}-${index}`} item={subItem} level={1} />
            ))}
          </ul>
        </div>
      );
    } else {
      // Category without children - show as heading with link
      return (
        <div style={{ breakInside: 'avoid', marginBottom: '20px' }}>
  ...
  // If there are nested children at level 1+, render them as sub-items
  return (
    <>
      <li>
        {isExternal ? (
  ...
  useEffect(() => {
    const loadMenus = async () => {
      try {
        ...
        const transformedMenus = transformMenuData(data, 1); // 1 for English
                      ...
                      <div id="mega-menu">
                        ...
                        >
                          {isLoadingMenu ? (
                            ...
                            <li>
                              <a className="dropdown" href="#">
                                <span className="menu-title">No menu items</span>
                              </a>
                            </li>
                          ) : (
                            menuItems.map((item, index) => (
                              <li key={`${item.title}-${index}`}>
                                ...
                                {item.items && item.items.length > 0 && (
                                  <div className="drop-menu" style={{ columnCount: 3, columnGap: '20px' }}>
          ...
          >
            ...
            <div
              className={`menu-header ${
                activeSubmenu || activeNestedSubmenu ? "with-back" : "centered"
              }`}
            >
              {activeNestedSubmenu ? (
                <button onClick={handleBackToSubmenu} className="back-button">
              ...
              <h2 className="menu-header-title">
                {activeNestedSubmenu || activeSubmenu || "Menu"}
              </h2>

              {(activeSubmenu || activeNestedSubmenu) && (
            ...
            <div
              className={`menu-content ${
                isMenuTransitioning ? "transitioning" : ""
              }`}
            >
              <ul
                className={`main-menu-list ${
                  !activeSubmenu && !activeNestedSubmenu ? "active" : ""
                }`}
              >
                {menuItems.map((item) => (
                  <li key={item.title} className="menu-item">
                    ...
                    <li key={subItem.title} className="menu-item">
                      {subItem.items ? (
                        <button
                          onClick={() => handleSubmenuItemClick(subItem.title)}
                          className={`menu-button ${
                            activeMenuItem === subItem.title ? "active" : ""
                          }`}
                        >
                          <span>{subItem.title}</span>
                          <FontAwesomeIcon icon={faChevronRight} />
              ...
              <ul
                className={`nested-submenu-list ${
                  activeNestedSubmenu ? "active" : ""
                }`}
              >
                {menuItems
                  .find((item) => item.title === activeSubmenu)
                  ?.items.find(
                    (subItem) => subItem.title === activeNestedSubmenu,
                  )
                  ?.items.map((linkItem) => (
                    <li key={linkItem.title} className="menu-item">
