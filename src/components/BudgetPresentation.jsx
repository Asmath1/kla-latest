...
import { fetchBudgetPresentation } from "../services/MasterService";
...
const BudgetPresentation = () => {
  ...
  // ── Active session info (for calendar) ───────────────────────────────────
  ...
  useEffect(() => {
    ...
    fetchBudgetPresentation()
      ...
      .catch((err) => {
        console.error("Failed to load budget presentation:", err);
        setError("Failed to load data. Please try again.");
      })
      .finally(() => setLoading(false));
  }, []);

  // ── Update calendar when KLA / session filter changes ────────────────────
  useEffect(() => {
  ...
  const rowsForKla = useMemo(
    () => allRows.filter((r) => Number(r.kla_id) === Number(klaId)),
    [allRows, klaId]
  );
      ...
      <div className="body_content">
        ...
        <BreadcrumbNav
          breadcrumbs={[
            ...
            { name: "Budget Presentation", href: "/budget-presentation" },
          ]}
        />

        <section className="Bussiness-schedule quest pt20 pb-30 pb30-md represent">
          <div className="container">
            <SectionTitle title="Budget Presentation" />
