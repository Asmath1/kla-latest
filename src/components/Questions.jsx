...
export const QuestionTabs = () => {
  ...
  const [selectedKLA, setSelectedKLA] = useState("15th KLA");
  ...
  const extractNumber = (text) => {
    if (!text) return null;
    const match = text.match(/\d+/); // gets first number
    return match ? Number(match[0]) : null;
  };
  const defaultLayoutPluginInstance = defaultLayoutPlugin();
  useEffect(() => {
    const klaId = extractNumber(selectedKLA); // "15th KLA" → 15
    const sessionNo = extractNumber(selectedSession); // "Session 1" → 1

    setFilters((prev) => ({
      ...prev,
      kla_id: klaId || prev.kla_id,
      session_type: sessionNo || "",
    }));
  }, [selectedKLA, selectedSession]);
  ...
  const tabFilterData = {
    ...
    ballot: {
      kla: ["15th KLA", "16th KLA", "14th KLA", "13th KLA", "12th KLA"],
  ...
  useEffect(() => {
    ...
    const loadBallotChart = async () => {
      try {
        const params = new URLSearchParams();
        params.append("kla_id", filters.kla_id || 15);

        if (filters.session_type && filters.session_type !== "All") {
          params.append("session_id", filters.session_type);
        }

        const url = `${API_ENDPOINTS.BALLOTCHART_LIST}?${params.toString()}`;
        ...
        if (json?.status && Array.isArray(json?.data)) {
          const mapped = json.data.map((item) => ({
            id: item.id,
            kla_id: Number(item.kla_id),
            kla: `${item.kla_id}${getOrdinalSuffix(item.kla_id)} KLA`,
            session_no: Number(item.session_no),
            session: item.session_name || `Session ${item.session_no}`,
            member: item.member_name || "", // safe
            date: item.date_of_presentation || "", // safe
            pdfUrl: item.pdf_link,
          }));

          setBallotData(mapped);
        }
      } catch (err) {
        console.error("Failed to load Ballot Chart", err);
      }
    };

    loadBallotChart();
  }, [filters.kla_id, filters.session_type]);

  const filteredBallotData = ballotData.filter((item) => {
    let ok = true;

    if (filters.kla_id) {
      ok = ok && item.kla_id === Number(filters.kla_id);
    }

    if (filters.session_type) {
      ok = ok && item.session_no === Number(filters.session_type);
    }

    if (selectedMember) {
      ok = ok && item.member === selectedMember;
    }

    if (selectedDate) {
      ok = ok && item.date === selectedDate;
    }
          ...
          >
            <div className="grids fpdf qst mb40-md">
              ...
              <div className="Qstn-allotmnt">
                <a
                  href="#"
                  className="rul qstn-allotment-section p-1 d-flex align-items-center w-100 justify-content-center"
                  onClick={() => openPdf(dummyPdf)}
                >
